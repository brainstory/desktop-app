...
pub fn spawn(app: AppHandle) {
	tauri::async_runtime::spawn(async move {
		loop {
			...
			let last_fired = state.db.get_setting("reminder_last_fired").unwrap_or_default();
			if last_fired == today {
				continue;
			}
			...
			let _ = app
				...
				.show();
			state.db.set_setting("reminder_last_fired", &today);
		}
	});
}
...
fn parse_time(time: &str) -> (u32, u32) {
	...
	let hour = parts.first().and_then(|h| h.parse::<u32>().ok()).unwrap_or(9).min(23);
	let minute = parts.get(1).and_then(|m| m.parse::<u32>().ok()).unwrap_or(0).min(59);
	(hour, minute)
}
