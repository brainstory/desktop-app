...
impl SttEngine {
	pub fn load(path: &Path, model_id: &str) -> Result<Self, String> {
		...
		Ok(Self { ctx: Arc::new(ctx), model_id: model_id.to_string() })
	}
...
/// Handles stereo->mono folding and naive linear resampling.
...
pub async fn transcribe_external(
	...
	let model = if model.is_empty() { "whisper-1".to_string() } else { model.to_string() };
